...
<body>
	<c:forEach var="bookMarkList" varStatus="status" items="${bookMarkList}">
		<input type="hidden" name="bookMarkList${status.index}" class="bookMarkList" value="${bookMarkList.sname }">
	</c:forEach>
	...
	<script>
		$(function(){
			$('.bookMarkList').each(function(index,element){
						...
						if(result != 0){
							...
							if(spaces>0&waitingNum==0){
								...
								var storeNode = $('<div class="store-container" style="display:flex; margin-bottom: 20px; width: 100%;justify-content: space-between;">' +
									    '<a href="storeDetail.do?sname=' + sname + '&genre='+category_name+'&address='+data[0].road_address_name+'&phone='+data[0].phone+'" style="text-decoration-line: none; color:black; width: 70%;">' +
									    ...
									    '<div class="place_name"><h3>' + data[0].place_name + '</h3></div>' +
									    '<div class="category_name">' + category_name + '</div>' +
									    '<div class="phone">' + data[0].phone +  '</div>' +
									    '<div class="road_address_name">' + data[0].road_address_name + '</div>' +
									    '</div>' +
									    '</div></a>' +
